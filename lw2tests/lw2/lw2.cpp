...
int main(int argc, char* argv[]) 
{
    ...
    std::vector<TimeThreads> vecTimes;
    TimeThreads tempTime;
      

    for (int threadsFor = 1; threadsFor <= 16;  threadsFor++)
    {
        ...
        std::cout << "BitDepth " << _InputBmp.TellBitDepth() << std::endl
            << "NumberOfColor " << _InputBmp.TellNumberOfColors() << std::endl
            << "Width " << _InputBmp.TellWidth() << std::endl
            << "Height " << _InputBmp.TellHeight() << std::endl;
        ...
        std::cout << "WritePicArrayToBMP" << std::endl;
        ...
        std::cout << "Изменения успешно применены и файл сохранен." << std::endl;
        auto end = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

        tempTime = { threadsFor, duration };
        vecTimes.push_back(tempTime);
        //std::cout << "Thread: " << numThreads << "Time: " << duration << std::endl;
        //return 0;
    }

    std::string filename = "time3.txt";
    std::ofstream outputFile(filename);

    for (const auto& line : vecTimes) 
    {
        outputFile << "Thread: " << line.numThread << " Time: " << line.timeThread << std::endl; // Записываем каждую строку
    }
